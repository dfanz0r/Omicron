...
public sealed class WorkspaceReadService : IWorkspaceReadService
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (wsPath is null)
        {
            return new WorkspaceReadErrorContent
            {
                ...
                Message =
                    $"Path escapes workspace root: '{path}' resolves outside '{_vfs.RootPath}'"
            };
        }

        FileStat? stat = await _vfs.StatAsync(wsPath.Value, ct);
        if (stat is null)
        {
            return new WorkspaceReadErrorContent
            {
        ...
        if (stat.IsDirectory)
        {
            IReadOnlyList<DirectoryEntry> allEntries = await _vfs.ReadDirectoryAsync(wsPath.Value, ct);
        ...
        ReadOnlyMemory<byte> bytes = await _vfs.ReadFileAsync(wsPath.Value, ct);
        ...
        (IReadOnlyList<WorkspaceTextLine> lines, int totalLines, bool truncated, int? nextOffset) =
            ParseLines(bytes, options);
    ...
    internal static (
        ...
        bool Truncated,
        int? NextOffset
        ) ParseLines(ReadOnlyMemory<byte> bytes, ReadOptions? options)
    {
        if (bytes.IsEmpty)
        {
        ...
        {
            int targetByte = options.Chunk.Value * DefaultChunkSizeBytes;
            int foundStart = -1;
        ...
        for (int i = startLine; i < endLine; i++)
        {
            ...
            if (
                result.Count >= MaxOutputLines
                || (byteCountWritten > 0 && byteCountWritten + lineBytes > MaxOutputBytes)
            )
            {
                break;
            }

            string lineText = Encoding.UTF8.GetString(span.Slice(lineByteStart, lineLength));
