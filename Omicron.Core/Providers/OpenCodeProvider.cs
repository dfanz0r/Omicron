...
public class OpenCodeProvider : ShapeBasedProvider
{
    ...
    public override string DefaultBaseUrl => _isGo
    ...
    public OpenCodeProvider(bool isGo = false, HttpClient? http = null) : base(http)
    {
        _isGo = isGo;
    ...
    protected override void SetAuthHeader(HttpRequestMessage request, string? apiKey)
    {
        ...
        {
            key = Environment.GetEnvironmentVariable(_apiKeyEnvVar!)
    ...
    {
        if (isGo) return ApiType.OpenAiChat;
    ...
    private static bool IsResponsesModel(string modelId) =>
        ResponsesModelIds.Contains(modelId);
}
