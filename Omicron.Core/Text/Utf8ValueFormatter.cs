...
namespace Omicron.Core.Text
{
    ...
    internal static partial class Utf8ValueFormatter
    {
        ...
        {
            written = 0;
            return TryFormatValue(value, destination, ref written, formatSpec, alignment: 0, provider);
        }
            ...
            IFormatProvider? provider)
        {
            ...
            {
                var genResult = TryFormatGenerated(ref value, destination.Slice(written), out var genWritten, formatSpec, provider);
            ...
            var formatter = formatSpec.IsEmpty ? FormatterCache<T>.Writer : null;
            ...
            if (value is IUtf8SpanFormattable formattable)
            {
                ...
                {
                    if (!formattable.TryFormat(destination.Slice(written), out var n, default, provider))
                        return Utf8FormatResult.InsufficientSpace;
                    written += n;
                }
                else
                {
                    ...
                    try { TranscodeFormatSpec(formatSpec, charBuf); }
                    catch (FormatException) { return Utf8FormatResult.UnsupportedType; }
                    if (!formattable.TryFormat(destination.Slice(written), out var n, charBuf, provider))
                        return Utf8FormatResult.InsufficientSpace;
                    written += n;
                }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static Utf8FormatResult ApplyAlignment(Span<byte> destination, ref int written, int before, int alignment)
        {
        ...
        internal static class FormatterCache<T>
        {
            ...
            private static bool FormatPlainString(string val, Span<byte> dest, out int w)
            {
                ...
                if (byteCount > dest.Length) { w = 0; return false; }
