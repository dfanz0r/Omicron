...
public struct Utf8Builder : IDisposable, IBufferWriter<byte>
{
    ...
    [ThreadStatic] private static bool _scratchBufferUsed;

    static Utf8Builder()
    {
        byte[] nl = Utf8NoBom.GetBytes(Environment.NewLine);
        if (nl.Length == 1)
        {
            _newLine1 = nl[0];
            _crlf = false;
    ...
    private void Grow(int sizeHint)
    {
        ...
        int nextSize = Math.Max(current * 2, needed);
    ...
    public void Append(char value, int repeatCount)
    {
        ...
        else
        {
            ...
            ReadOnlySpan<char> chars = stackalloc char[1]
            {
                value
            };
    ...
    public void Append<T>(T value)
    {
        ...
        Utf8FormatResult result = Utf8ValueFormatter.TryFormat(value, scratch, out int written);
    ...
    {
        Span<byte> span = writer.GetSpan(_index);
        AsSpan().CopyTo(span);
    ...
    {
        throw new InvalidOperationException("A thread-static Utf8Builder is already in use on this thread. "
