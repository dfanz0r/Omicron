...
public static class GraphemeSegmenter
{
    ...
    public static List<GraphemeCluster> SegmentUtf8(ReadOnlySpan<byte> utf8)
    {
        ...
        {
            OperationStatus status = Rune.DecodeFromUtf8(utf8[offset..], out Rune rune, out int consumed);
            if (status != OperationStatus.Done)
            {
                ...
                {
                    result.Add(new GraphemeCluster(clusterStartOffset,
            ...
            if (previousRune.HasValue)
            {
                ...
                // Regional indicator pair: both are regional indicators
                    ...
                    IsRegionalIndicator(previousRune.Value.Value)
                    && IsRegionalIndicator(runeValue)
                    && runeCount < 2
                )
                {
                    continues = true;
    ...
    private static bool IsCombiningMark(int code)
    {
        ...
        return CellWidthCalculator.GetWidth(code) == 0 && code > 0x0300;
    }
