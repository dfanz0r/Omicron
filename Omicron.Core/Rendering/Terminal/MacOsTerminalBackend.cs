...
public sealed class MacOsTerminalBackend : ITerminalBackend
{
    ...
    private int _stdinFd = MacOsNative.STDIN_FILENO;
    private int _stdoutFd = MacOsNative.STDOUT_FILENO;
    ...
    private void InitializeTerminalMode()
    {
        if (MacOsNative.tcgetattr(_stdinFd, out _originalTermios) == 0)
        {
            ...
            raw.c_lflag &= ~(MacOsNative.ECHO | MacOsNative.ECHOE | MacOsNative.ECHOK |
                             MacOsNative.ECHONL | MacOsNative.ICANON | MacOsNative.IEXTEN);
            // Keep ISIG for Ctrl+C
            raw.c_lflag |= MacOsNative.ISIG;

            raw.c_iflag &= ~(MacOsNative.ICRNL | MacOsNative.INLCR | MacOsNative.IXON |
                             MacOsNative.IXOFF | MacOsNative.IXANY | MacOsNative.IGNBRK |
                             MacOsNative.BRKINT | MacOsNative.PARMRK | MacOsNative.ISTRIP |
                             MacOsNative.INPCK | MacOsNative.IGNPAR);
            raw.c_iflag |= MacOsNative.IUTF8;

            raw.c_oflag &= ~(MacOsNative.OPOST | MacOsNative.ONLCR | MacOsNative.OCRNL |
                             MacOsNative.ONOCR | MacOsNative.ONLRET);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            bool ready = PollStdin(cancellationToken, timeoutMs: 10);
        ...
        while (true)
        {
            ...
            if (result > 0)
            {
                if ((fd.revents & (MacOsNative.POLLIN | MacOsNative.POLLPRI)) != 0)
                    return true;
                if ((fd.revents & (MacOsNative.POLLERR | MacOsNative.POLLHUP | MacOsNative.POLLNVAL)) != 0)
                    return false;
            }
            else if (result == 0)
            {
    ...
    private void RefreshSize()
    {
        try
        {
            ...
            if (RuntimeInformation.ProcessArchitecture == Architecture.Arm64)
                result = MacOsNative.ioctl_arm64(_stdoutFd, MacOsNative.TIOCGWINSZ, 0, 0, 0, 0, 0, 0, ref wsz);
            else
            ...
            else
            {
                ...
                if (RuntimeInformation.ProcessArchitecture == Architecture.Arm64)
                    result = MacOsNative.ioctl_arm64(_stdinFd, MacOsNative.TIOCGWINSZ, 0, 0, 0, 0, 0, 0, ref wsz);
                else
