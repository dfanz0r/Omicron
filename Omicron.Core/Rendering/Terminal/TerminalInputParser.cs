...
public sealed class TerminalInputParser
{
        ...
        while (offset < totalLength)
        {
            ...
            if (_accumulatingPaste)
            {
                ...
                else
                {
                    ...
                    if (preserveLen > 0)
                    {
                        _readBuffer.AsSpan(totalLength - preserveLen, preserveLen).CopyTo(_readBuffer);
            ...
            var status = System.Buffers.OperationStatus.Done;
            Rune rune = default;
            int consumed = 0;

            try
            {
                status = Rune.DecodeFromUtf8(
                    _readBuffer.AsSpan(offset, totalLength - offset),
                    out rune, out consumed);
            }
            catch
            {
                // Skip bad byte
            ...
            if (status == System.Buffers.OperationStatus.Done)
            {
                ...
                if (rune.Value is '\r' or '\n')
                {
                    ...
                    {
                        var enterModifiers = KeyModifiers.None;
                        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows) && WindowsModifierFallbackEnabled)
                        {
    ...
    {
        if (_pendingCount == 1 && _readBuffer[0] == 0x1B &&
            _pendingEscapeSince != DateTime.MinValue &&
    ...
    private void EnsureReadBufferCapacity(int required)
    {
        ...
        {
            var newBuffer = new byte[Math.Max(_readBuffer.Length * 2, required)];
    ...
    private bool TryParseEscapeSequence(byte[] buffer, ref int offset, int length, out TerminalEvent? terminalEvent)
    {
        ...
        if (buffer[offset + 1] == '[')
        {
                ...
                int.TryParse(parts[2], out var otherKeyCode))
            {
                ...
                {
                    try { rune = new Rune(otherKeyCode); }
    ...
    internal static bool TryParseKittyKeySequence(byte[] buffer, ref int offset, int length, out TerminalEvent? terminalEvent)
    {
        ...
        if (key == Key.Character)
        {
            ...
            {
                var enumerator = resolvedText.EnumerateRunes().GetEnumerator();
