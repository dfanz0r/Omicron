...
public readonly record struct GlyphRef
{
    ...
    public static GlyphRef Ascii(byte ascii)
    {
        return new GlyphRef(ascii);
    }

    /// <summary>Create a glyph reference from an intern table ID.</summary>
    public static GlyphRef Interned(int internId)
    {
        return new GlyphRef((uint)internId | 0x80000000);
    }
}
