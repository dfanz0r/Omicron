...
public sealed class ProviderStateManager : IProviderStateManager
{
    ...
    public ProviderStateManager(IProviderConversationStateStore store, IEventSink? eventSink = null)
    {
    ...
    public ProviderTurnState Set(ProviderTurnState state, string? reason = null)
    {
        ...
        ProviderStateKey normalizedKey = state.Key.Normalize();
        ProviderTurnState normalizedState = state with
        {
            Key = normalizedKey
        };
        _store.Set(normalizedState);
        _eventSink?.Emit(new ProviderStateUpdatedEvent(EventEnvelope.ForSession(normalizedKey.SessionId),
            new ProviderStateSnapshot(normalizedKey,
                normalizedState.PreviousResponseId,
                normalizedState.ConversationId,
                normalizedState.SessionAffinityKey,
                normalizedState.ProviderMetadata,
                normalizedState.StoragePolicy),
            reason));
    ...
    {
        bool removed = _store.Clear(key);
        if (removed)
        {
            ProviderStateKey normalizedKey = key.Normalize();
            _eventSink?.Emit(new ProviderStateClearedEvent(EventEnvelope.ForSession(normalizedKey.SessionId),
                normalizedKey,
                reason));
        }
    }
    ...
    {
        IReadOnlyList<ProviderStateKey> removedKeys = _store.ClearSession(sessionId);
        foreach (ProviderStateKey key in removedKeys)
        {
            _eventSink?.Emit(new ProviderStateClearedEvent(EventEnvelope.ForSession(sessionId), key, reason));
        }
    }

    public IReadOnlyList<ProviderStateKey> GetSessionKeys(SessionId sessionId)
    {
        return _store.GetSessionKeys(sessionId);
    }
}
