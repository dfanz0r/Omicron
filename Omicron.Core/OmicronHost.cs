...
public sealed class OmicronHost : IDisposable
{
    ...
    public async ValueTask<int> RefreshModelMetadataAsync(CancellationToken ct = default)
    {
        ...
        {
            new StaticModelMetadataSource(ModelCatalog), new ModelsDevMetadataSource(_metadataHttp)
        };
        ...
        CancellationToken ct = default)
    {
        ...
        var session = new AgentSession(config, Tools, Permissions, Events, ProviderStateManager);

        var request = new SessionCreateRequest(config.Model.Id,
    ...
    {
        return CreateSessionAsync(config).GetAwaiter().GetResult();
    }
    ...
    {
        return CreateSession(SessionConfig.Create(model,
            systemPrompt,
            apiKey,
            maxTokens,
            temperature,
        ...
        CancellationToken ct = default)
    {
        ...
        SessionRecord? record = await SessionStore.GetSessionAsync(request.SessionId, ct);
        if (record is null)
        {
            return null;
        }
        ...
        await foreach (
            OmicronEvent e in SessionStore.ReadEventsAsync(request.SessionId, EventSequenceRange.All, ct)
        )
        {
            events.Add(e);
        }
        ...
        ModelMetadata targetMeta =
            ModelCatalog.GetMetadata(request.Model) ?? BuildFallbackMetadata(request.Model);
        IReadOnlyList<ModalityValidationError> errors = CheckModalityCompatibility(events, targetMeta);
        if (errors.Count > 0)
        {
            string msg = FormatModalityErrors(request.Model.Id, errors);
        ...
        bool sameProvider = string.Equals(record.ProviderName,
            request.Model.ProviderName,
            StringComparison.OrdinalIgnoreCase);
        bool restoreProvider =
            sameProvider
            && record.ModelId == request.Model.Id
            && record.ApiType == request.Model.ApiType;

        var config = SessionConfig.Create(request.Model, record.SystemPrompt, request.ApiKey);
        return AgentSession.FromProjection(projection,
            config,
            request.SessionId,
            Tools,
            Permissions,
    ...
    public Task<AgentSession?> ResumeSessionAsync(
        ...
        CancellationToken ct = default)
    {
        return ResumeSessionAsync(new SessionResumeRequest(sessionId, model, apiKey), ct);
    }
        ...
        CancellationToken ct = default)
    {
        ...
        var events = new List<OmicronEvent>();
        await foreach (
            OmicronEvent e in SessionStore.ReadEventsAsync(request.SourceSessionId,
                EventSequenceRange.All,
                ct)
        )
        {
            events.Add(e);
        ...
        ModelMetadata forkTargetMeta =
            ModelCatalog.GetMetadata(request.Model) ?? BuildFallbackMetadata(request.Model);
        IReadOnlyList<ModalityValidationError> forkErrors = CheckModalityCompatibility(events, forkTargetMeta);
        if (forkErrors.Count > 0)
        {
            string forkMsg = FormatModalityErrors(request.Model.Id, forkErrors);
        ...
        SessionRecord? sourceRecord = await SessionStore.GetSessionAsync(request.SourceSessionId, ct);
        string? forkSystemPrompt = request.SystemPrompt ?? sourceRecord?.SystemPrompt;
        var createRequest = new SessionCreateRequest(request.Model.Id,
            request.Model.ProviderName,
        ...
        List<OmicronEvent> replayEvents = BuildForkReplayEvents(session, request.Model, projection);
    ...
    public Task<AgentSession?> ForkSessionAsync(
        ...
        CancellationToken ct = default)
    {
        return ForkSessionAsync(new SessionForkRequest(sourceId, model, systemPrompt, apiKey), ct);
    }
        ...
        ModelMetadata targetModel)
    {
        ...
        IEnumerable<IGrouping<string, ModalityUsedEvent>> byModality = modalityEvents.GroupBy(e => e.Modality);
        var errors = new List<ModalityValidationError>();

        foreach (IGrouping<string, ModalityUsedEvent> group in byModality)
        {
            ...
            if (!supported)
            {
                errors.Add(new ModalityValidationError(group.Key,
    ...
    private static ModelMetadata BuildFallbackMetadata(Model model)
    {
        ...
        return new ModelMetadata(model.Id,
            model.ProviderName,
            SupportsVision: model.SupportsImages ? true : null,
            Modalities: modalities);
    }

    private static string FormatModalityErrors(
        string modelId,
        IReadOnlyList<ModalityValidationError> errors)
    {
        ...
        foreach (ModalityValidationError error in errors)
        {
            sb.AppendLine(
                $"  - {error.Modality} ({error.AffectedFiles.Count} file(s): {string.Join(", ", error.AffectedFiles)})");
        }
    ...
    private static List<OmicronEvent> BuildForkReplayEvents(
        AgentSession session,
        Model model,
        SessionProjection projection)
    {
        var replayEvents = new List<OmicronEvent>();
        replayEvents.Add(new SessionStartedEvent(EventEnvelope.ForSession(session.Id),
            session.AgentId,
            model.Id,
            model.ProviderName));

        foreach (Message msg in projection.Messages)
        {
            switch (msg.Role)
            {
                case MessageRole.User:
                    replayEvents.Add(new UserMessageEvent(EventEnvelope.ForSession(session.Id),
                ...
                case MessageRole.Assistant:
                    {
                        ...
                        if (toolCalls is { Count: > 0 })
                        {
                            ...
                            {
                                replayEvents.Add(new ToolInvocationStartedEvent(EventEnvelope.ForSession(session.Id),
                        ...
                        replayEvents.Add(new AssistantResponseCompleteEvent(EventEnvelope.ForSession(session.Id),
                            msg.TextData ?? Utf8String.Empty,
                            msg.ReasoningData,
                            new TokenUsage(0, 0)));
                        break;
                ...
                case MessageRole.ToolResult:
                    replayEvents.Add(new ToolInvocationCompletedEvent(EventEnvelope.ForSession(session.Id),
