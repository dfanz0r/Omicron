...
public sealed record Message
{
    ...
    /// <summary>
    ///     Whether this message has reasoning data (including empty reasoning).
    ///     Use <see cref="GetReasoningStringOrNull()" /> to distinguish null (no reasoning)
    ///     from empty (explicit empty reasoning).
    /// </summary>
    ...
    public ReadOnlySpan<byte> TextUtf8 =>
        TextData is not null ? TextData.Utf8Span : ReadOnlySpan<byte>.Empty;

    /// <summary>UTF-8 span of the reasoning text (empty if no reasoning). Zero-alloc.</summary>
    public ReadOnlySpan<byte> ReasoningUtf8 =>
        ReasoningData is not null ? ReasoningData.Utf8Span : ReadOnlySpan<byte>.Empty;
    ...
    {
        return ReasoningData?.ToString() ?? string.Empty;
    }

    /// <summary>Reasoning string that preserves null-vs-empty: null when no reasoning, string when present.</summary>
    public string? GetReasoningStringOrNull()
    {
        return ReasoningData?.ToString();
    }
    ...
    public static Message UserMessage(string text, IReadOnlyList<ImageContent>? images = null)
    {
        return new Message
        {
            ...
            TextData = Utf8String.FromString(text),
            Images = images
        };
    }
    ...
    public static Message ToolResultMessage(
        string toolCallId,
        string toolName,
        string text,
        bool isError = false)
