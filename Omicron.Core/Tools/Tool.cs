...
public static class ToolSchema
{
        ...
        string[]? required = null)
    {
        ...
        foreach ((string name, JsonElement schema) in properties)
        {
    ...
    {
        return JsonDocument
            .Parse($$"""{"type":"string","description":"{{description}}"}""")
            .RootElement.Clone();
    }
    ...
    {
        return JsonDocument
            .Parse($$"""{"type":"integer","description":"{{description}}"}""")
            .RootElement.Clone();
    }
    ...
    {
        return JsonDocument
            .Parse($$"""{"type":"boolean","description":"{{description}}"}""")
            .RootElement.Clone();
    }
    ...
    {
        return JsonDocument
            .Parse($$"""{"type":"array","description":"{{description}}","items":{{items.GetRawText()}}}""")
            .RootElement.Clone();
    }

    public static JsonElement EnumProperty(string description, string[] values)
    {
        string items = string.Join(",", values.Select(v => $"\"{v}\""));
        return JsonDocument
            .Parse($$"""{"type":"string","description":"{{description}}","enum":[{{items}}]}""")
            .RootElement.Clone();
    }
}
