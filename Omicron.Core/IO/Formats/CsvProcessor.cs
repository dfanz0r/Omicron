...
public sealed class CsvProcessor : IContentProcessor
{
        ...
        CancellationToken ct = default)
    {
        ...
        string delimiterStr = new(delimiter, 1);
        ...
        try
        {
            ...
            try
            {
                ...
                while (rowCount < MaxPreviewRows && byteBudget > 0)
                {
                    ...
                    using Utf8Builder rowBuilder = Utf8Text.CreateBuilder();
                    ...
                    int rowBytes = rowBuilder.Length;
                    ...
                    int totalBytes = RowPrefixBytes + rowBytes;
            ...
            if (totalLines > 0)
            {
                ...
                if (headerLen > 0 && span[headerStart + headerLen - 1] == (byte)'\r')
                {
                    headerLen--;
                }

                string headerText = Encoding.UTF8.GetString(span.Slice(headerStart, headerLen));
                string[] columns = headerText.Split(delimiter);
                Utf8CompositeFormat.AppendFormatUtf8Slow(ref output,
                    "Columns ({0}): "u8,
                    columns.Length);
            ...
            for (int i = 1; i <= showRows; i++)
            {
                ...
                if (lineLen > 0 && span[lineStart + lineLen - 1] == (byte)'\r')
                {
                    lineLen--;
                }
