...
public sealed class ArchiveProcessor : IContentProcessor
{
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            List<string>? entries = TryListContents(bytes.Span, ext);
            if (entries is not null)
            {
                Utf8CompositeFormat.AppendFormatUtf8Slow(ref output,
                    "Contents ({0} entries):"u8,
                    entries.Count);
                output.AppendLine();
                output.AppendLine();
    ...
    private static List<string>? TryListContents(ReadOnlySpan<byte> bytes, string ext)
    {
        ...
        try
        {
            ...
            using IArchive archive = ArchiveFactory.OpenArchive(ms,
                new ReaderOptions());
