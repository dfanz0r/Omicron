...
public sealed class AudioProcessor : IContentProcessor
{
        ...
        CancellationToken ct = default)
    {
        ...
        AudioMeta? meta = ParseAudioMetadata(context.RelativePath, context.Bytes.Span);
        string mimeType = GuessAudioMimeType(context.RelativePath);
        ...
        if (audioCapable)
        {
            ...
            try
            {
                Utf8CompositeFormat.AppendFormatUtf8Slow(ref output,
                    ...
                    FormatSize.Format(context.Bytes.Length),
                    mimeType);
                output.AppendLine();
                if (meta is not null)
                {
                    Utf8CompositeFormat.AppendFormatUtf8Slow(ref output,
                        "Format: {0}, Duration: {1:F1}s, Bitrate: {2} kbps, Sample Rate: {3} Hz"u8,
                        meta.Format,
                        meta.Duration,
                        meta.Bitrate,
                        meta.SampleRate);
        ...
        Utf8Builder audioMeta = Utf8Text.CreateBuilder();
        try
        {
            Utf8CompositeFormat.AppendFormatUtf8Slow(ref audioMeta,
                "[FILE] {0}  ({1}, {2})"u8,
                context.RelativePath,
                FormatSize.Format(context.Bytes.Length),
                mimeType);
            ...
            if (meta is not null)
            {
                ...
                if (meta.Duration > 0)
                {
                    Utf8CompositeFormat.AppendFormatUtf8(ref audioMeta,
                        "Duration: {0:F1}s"u8,
                        meta.Duration);
                    audioMeta.AppendLine();
                }
    ...
    private static AudioMeta? ParseMp3Header(ReadOnlySpan<byte> bytes)
    {
        ...
        for (int i = 0; i < Math.Min(bytes.Length, 4096); i++)
        {
            if (i + 2 < bytes.Length && bytes[i] == 0xFF && (bytes[i + 1] & 0xE0) == 0xE0)
            {
                ...
                int sampleRate =
                    sampleRateIndex < sampleRates.Length ? sampleRates[sampleRateIndex] : 0;
...
public sealed class VideoProcessor : IContentProcessor
{
        ...
        CancellationToken ct = default)
    {
        ...
        VideoMeta? meta = ParseVideoMetadata(context.RelativePath, context.Bytes.Span);
        string mimeType = GuessVideoMimeType(context.RelativePath);
        ...
        // Text-only model: metadata only
        Utf8Builder videoMeta = Utf8Text.CreateBuilder();
        try
        {
            Utf8CompositeFormat.AppendFormatUtf8Slow(ref videoMeta,
                "[FILE] {0}  ({1}, {2})"u8,
                context.RelativePath,
                FormatSize.Format(context.Bytes.Length),
                mimeType);
            ...
            if (meta is not null)
            {
                ...
                if (meta.Duration > 0)
                {
                    Utf8CompositeFormat.AppendFormatUtf8(ref videoMeta,
                        "Duration: {0:F1}s"u8,
                        meta.Duration);
                    videoMeta.AppendLine();
                }
    ...
    private static VideoMeta? ParseAviHeader(ReadOnlySpan<byte> bytes)
    {
        ...
        if (bytes[36] == 0x61 && bytes[37] == 0x76 && bytes[38] == 0x69 && bytes[39] == 0x68)
        {
            ...
            double duration =
                microsecPerFrame > 0 ? totalFrames * microsecPerFrame / 1_000_000.0 : 0;
    ...
    internal sealed record VideoMeta(
        string Format,
        double Duration,
        int Width,
        int Height,
