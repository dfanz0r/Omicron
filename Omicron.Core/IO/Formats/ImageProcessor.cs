...
public sealed class ImageProcessor : IContentProcessor
{
    ...
    public OutputModality OutputModality =>
        OutputModality.ImageBase64 | OutputModality.HexDump | OutputModality.Metadata;
        ...
        CancellationToken ct = default)
    {
        ...
        string mimeType = GuessMimeType(context.RelativePath, bytes.Span);
        (int Width, int Height)? dim = TryGetDimensions(bytes.Span);
    ...
    internal static (int Width, int Height)? TryGetDimensions(ReadOnlySpan<byte> bytes)
    {
        ...
        try
        {
            ...
            if (
                bytes.Length >= 8
                && (
                    (bytes[0] == 0x4D && bytes[1] == 0x4D) || (bytes[0] == 0x49 && bytes[1] == 0x49)
                )
            )
            {
                bool bigEndian = bytes[0] == 0x4D;
                if (
                    (bigEndian && bytes[2] == 0x00 && bytes[3] == 0x2A)
                ...
                )
                {
                    ...
                    if (ifdOffset > 0 && ifdOffset + 12 < bytes.Length)
                    {
                        ...
                        for (int e = 0; e < entryCount && e < 20; e++)
                        {
                            ...
                            {
                                width = bigEndian
                                    ? (bytes[tagOffset + 8] << 24)
                                      | (bytes[tagOffset + 9] << 16)
                                      | (bytes[tagOffset + 10] << 8)
                                      | bytes[tagOffset + 11]
                                    : bytes[tagOffset + 8]
                                      | (bytes[tagOffset + 9] << 8)
                                      | (bytes[tagOffset + 10] << 16)
                                      | (bytes[tagOffset + 11] << 24);
                            }
