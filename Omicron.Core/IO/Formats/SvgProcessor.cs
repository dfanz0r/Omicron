...
public sealed class SvgProcessor : IContentProcessor
{
        ...
        CancellationToken ct = default)
    {
        ...
        ReadOnlyMemory<byte> slice = bytes.Length > MaxSvgBytes ? bytes[..(int)MaxSvgBytes] : bytes;
        ...
        try
        {
            if (isTruncated)
            {
                Utf8CompositeFormat.AppendFormatUtf8Slow(ref output,
                    "[FILE] {0}  ({1}, SVG — file too large, showing first {2})"u8,
                    context.RelativePath,
                    FormatSize.Format(bytes.Length),
                    FormatSize.Format(MaxSvgBytes));
            }
            else
            {
                Utf8CompositeFormat.AppendFormatUtf8Slow(ref output,
                    "[FILE] {0}  ({1} lines, SVG)"u8,
                    context.RelativePath,
                    totalLines);
            }
