...
public static class TextEncodingDetector
{
    ...
    public static bool IsText(ReadOnlySpan<byte> bytes)
    {
        ...
        var sample = bytes.Length <= MaxDetectBytes
            ? bytes
            : bytes[..MaxDetectBytes];
        ...
        var decoded = Utf8Lenient.GetString(sample);
    ...
    public static bool IsTextFile(string absolutePath)
    {
        try
        {
            using var fs = new FileStream(absolutePath, FileMode.Open, FileAccess.Read, FileShare.Read, MaxDetectBytes);
