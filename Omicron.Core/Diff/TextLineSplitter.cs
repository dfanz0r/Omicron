...
internal static class TextLineSplitter
{
    ...
    public static Utf8LineIndex CreateUtf8LineIndex(ReadOnlyMemory<byte> utf8Bytes)
        => Utf8LineIndex.Create(utf8Bytes);
    ...
    /// </summary>
    public static bool TryReadNextLine(ReadOnlySpan<char> text, ref int index, out ReadOnlySpan<char> line)
    {
        ...
        var newlineOffset = remaining.IndexOf('\n');
    ...
    /// </summary>
    public static bool TryReadNextLine(ReadOnlySpan<byte> utf8Bytes, ref int index, out ReadOnlySpan<byte> line)
    {
...
internal readonly struct Utf8LineIndex
{
    ...
    public static Utf8LineIndex Empty { get; } = new(ReadOnlyMemory<byte>.Empty, Array.Empty<int>());
    ...
    public ReadOnlySpan<byte> GetLine(int index)
    {
        ...
        var end = index + 1 < _lineStarts.Length ? _lineStarts[index + 1] : span.Length;
...
internal readonly struct Utf8LineKey : IEquatable<Utf8LineKey>
{
    ...
    public bool Equals(Utf8LineKey other)
        => _hash == other._hash && Span.SequenceEqual(other.Span);

    public override bool Equals(object? obj)
        => obj is Utf8LineKey other && Equals(other);
