...
{
    /// <summary>Normalize an edit list, running multiple passes until stable.
    /// Merges adjacent delete+insert and insert+delete pairs into single replacement edits,
    /// and merges adjacent edits of the same position into replacements.</summary>
    public static List<TextDiffEdit> Normalize(List<TextDiffEdit> edits)
    {
        if (edits.Count <= 1) return edits;
        ...
        do
        {
            ...
            while (i < edits.Count)
            {
                if (i + 1 < edits.Count && CanMerge(edits[i], edits[i + 1], out var merged))
                {
                    result.Add(merged);
                    i += 2;
                    changed = true;
                }
                else if (i + 2 < edits.Count
                    && edits[i].IsInsert && edits[i + 1].IsDelete && edits[i + 2].IsInsert
    ...
    private static bool CanMerge(TextDiffEdit a, TextDiffEdit b, out TextDiffEdit merged)
    {
        ...
        if (a.IsDelete
            && a.OldStart + a.OldCount == b.OldStart
        ...
        if (a.IsInsert
            ...
            && a.NewStart + a.NewCount == b.NewStart)
        {
