...
internal static class TextDiffEngine
{
        ...
        TextDiffOptions? options = null)
    {
        ...
        int[] oldCodes = new int[oldLines.Count];
        int[] newCodes = new int[newLines.Count];
        for (int i = 0; i < oldLines.Count; i++)
        {
            oldCodes[i] = normalizer.GetCode(oldLines[i]);
        }

        for (int i = 0; i < newLines.Count; i++)
        {
            newCodes[i] = normalizer.GetCode(newLines[i]);
        }
    ...
    {
        int N = oldCodes.Length;
        int M = newCodes.Length;
        ...
        if (N > maxLines || M > maxLines)
        {
            return new TextDiffResult([],
                ...
                M,
                true,
                $"Input exceeds MaxLineCount ({maxLines}). Old={N}, New={M}");
        }
        ...
        if (edits is null)
        {
            ...
            return new TextDiffResult([],
                ...
                M,
                true,
                $"All diff strategies declined. Old={N}, New={M}");
        }
    ...
    internal sealed class LineNormalizer
    {
        ...
        public LineNormalizer(TextDiffOptions options)
        {
            _options = options;
        }
        ...
        {
            string key = Normalize(line, _options);

            if (!_codes.TryGetValue(key, out int code))
            {
                code = _nextCode++;
                _codes[key] = code;
            }
    ...
    internal sealed class Utf8LineNormalizer
    {
        ...
        public Utf8LineNormalizer(TextDiffOptions options)
        {
            _options = options;
        }
        ...
        public int GetCode(Utf8LineIndex lines, int index)
        {
            ...
            {
                Utf8LineKey key = lines.GetKey(index);
                if (!_utf8Codes.TryGetValue(key, out int code))
                {
                    code = _nextCode++;
                    _utf8Codes[key] = code;
                }
            ...
            string normalized = LineNormalizer.Normalize(Encoding.UTF8.GetString(lines[index]),
                _options);
            if (!_stringCodes.TryGetValue(normalized, out int stringCode))
            {
