...
public class ModelMetadataTests
{
    ...
    {
        var merger = new ModelMetadataMerger();
        var source = new List<ModelMetadata> { new("gpt-4o", "openai", DisplayName: "GPT-4o") };
    ...
    {
        var merger = new ModelMetadataMerger();
        var baseline = new List<ModelMetadata> { new("gpt-4o", "openai", DisplayName: "GPT-4o", ContextWindow: 8000) };
        var overrides = new List<ModelMetadata> { new("gpt-4o", "openai", DisplayName: "Override", ContextWindow: 128000) };
        var result = merger.Merge(baseline, overrides);
    ...
    public void Merge_NullFieldsDoNotOverwrite()
    {
        var merger = new ModelMetadataMerger();
        var baseline = new List<ModelMetadata> { new("gpt-4o", "openai", DisplayName: "GPT-4o", ContextWindow: 8000) };
        var overrides = new List<ModelMetadata> { new("gpt-4o", "openai") };
        var result = merger.Merge(baseline, overrides);
    ...
    public void Merge_DifferentModels_Combines()
    {
        var merger = new ModelMetadataMerger();
        var result = merger.Merge(
            new List<ModelMetadata> { new("gpt-4o", "openai") },
            new List<ModelMetadata> { new("claude-3", "anthropic") });
    ...
    public void Merge_PartialOverride_PreservesUnchangedFields()
    {
        var merger = new ModelMetadataMerger();
        var result = merger.Merge(
            new List<ModelMetadata> { new("gpt-4o", "openai", DisplayName: "GPT-4o", ContextWindow: 8000, SupportsVision: true) },
            new List<ModelMetadata> { new("gpt-4o", "openai", ContextWindow: 128000) });
        Assert.Equal("GPT-4o", result[0].DisplayName);
        Assert.Equal(128000, result[0].ContextWindow);
    ...
    public void Merge_CaseInsensitiveModelId()
    {
        var merger = new ModelMetadataMerger();
        var result = merger.Merge(
            new List<ModelMetadata> { new("GPT-4o", "openai", DisplayName: "GPT-4o") },
            new List<ModelMetadata> { new("gpt-4o", "openai", ContextWindow: 128000) });
        Assert.Single(result);
        Assert.Equal(128000, result[0].ContextWindow);
    }
    ...
    {
        var merger = new ModelMetadataMerger();
        var result = merger.Merge(null!, new List<ModelMetadata> { new("gpt-4o", "openai") });
        Assert.Single(result);
    }
    ...
    public async Task StaticSource_IncludesContextWindow()
    {
        ...
        var source = new StaticModelMetadataSource(catalog);
        var metadata = await source.LoadAsync();
        var gpt4o = metadata.First(m => m.ModelId == "gpt-4o");
    ...
    public async Task Catalog_RefreshMetadata_EmptySources_ReturnsZero()
    {
        var providerRegistry = new Omicron.Core.Providers.ProviderFactory();
        var catalog = new ModelCatalogService(providerRegistry);
        var count = await catalog.RefreshMetadataAsync(Array.Empty<IModelMetadataSource>());
    ...
    public async Task Catalog_RefreshMetadata_LayersFromStaticSource()
    {
        ...
        var count = await catalog.RefreshMetadataAsync(new IModelMetadataSource[] { staticSource });
        Assert.True(count > 0);
    ...
    public async Task Catalog_RefreshMetadata_OverlayWorks()
    {
        ...
        var catalog = new ModelCatalogService(providerRegistry);
        var staticSource = new StaticModelMetadataSource(catalog);
        await catalog.RefreshMetadataAsync(new IModelMetadataSource[] { staticSource });

        var overlay = new List<ModelMetadata> { new("gpt-4o", "openai", ContextWindow: 999999) };
        var overlaySource = new StaticOverlaySource(overlay);
        await catalog.RefreshMetadataAsync(new IModelMetadataSource[] { staticSource, overlaySource });
    ...
    public async Task Catalog_RefreshMetadata_FailingSource_DoesNotThrow()
    {
        var providerRegistry = new Omicron.Core.Providers.ProviderFactory();
        var catalog = new ModelCatalogService(providerRegistry);
        var count = await catalog.RefreshMetadataAsync(new IModelMetadataSource[] { new ThrowingMetadataSource() });
        Assert.Equal(0, count);
    }
    ...
    public async Task Catalog_RefreshMetadata_PreservesExistingAfterEmptyRefresh()
    {
        ...
        var staticSource = new StaticModelMetadataSource(catalog);
        await catalog.RefreshMetadataAsync(new IModelMetadataSource[] { staticSource });
        var before = catalog.Metadata.Count;
        Assert.True(before > 0);

        var count = await catalog.RefreshMetadataAsync(new IModelMetadataSource[] { new ModelsDevMetadataSource(new HttpClient(new FakeHandler("{}"))) });
    ...
    public async Task Catalog_RefreshMetadata_PropagatesCancellation()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() =>
            catalog.RefreshMetadataAsync(new IModelMetadataSource[] { new CancellingMetadataSource() }, cts.Token).AsTask());
    }
    ...
    {
        var json = @"{""openai"":{""gpt-4o"":{""name"":""GPT-4o"",""limit"":{""context"":128000,""output"":16384}}}}";
        var doc = JsonDocument.Parse(json);
        var result = ModelsDevMetadataSource.Parse(doc.RootElement);
        Assert.NotEmpty(result);
        var gpt4o = result.First(m => m.ModelId == "gpt-4o");
    ...
    {
        var json = @"{""openai"":{""gpt-4o"":{""tool_call"":true,""reasoning"":true,""structured_output"":true,""attachment"":true}}}";
        var doc = JsonDocument.Parse(json);
        var result = ModelsDevMetadataSource.Parse(doc.RootElement);
        var gpt4o = result.First(m => m.ModelId == "gpt-4o");
    ...
    {
        var json = @"{""anthropic"":{""anthropic/claude-sonnet-4"":{""name"":""Claude Sonnet 4""}}}";
        var doc = JsonDocument.Parse(json);
        var result = ModelsDevMetadataSource.Parse(doc.RootElement);
        Assert.NotEmpty(result);
        var claude = result.First();
    ...
    public void Extension_GetMetadata_ReturnsNullForUnknown()
    {
        ...
        var model = new Model { Id = "unknown", ProviderName = "none" };
    ...
    public async Task Extension_GetMetadata_ReturnsAfterRefresh()
    {
        ...
        {
            new("test-model", "test-prov", DisplayName: "Test", ContextWindow: 99999, MaxOutputTokens: 5000)
        };
        await svc.RefreshMetadataAsync(new IModelMetadataSource[] { new StaticOverlaySource(meta) });
    ...
    public void Extension_GetEffectiveContextWindow_NullWhenNoData()
    {
        var prov = new Omicron.Core.Providers.ProviderFactory();
        var catalog = new ModelCatalogService(prov) as IModelCatalog;
        var model = new Model { Id = "m", ProviderName = "p", ContextWindow = 0 };
    ...
    public void ModelsDevSource_Parse_PreservesRoutedModelId()
    {
        ...
        var json = @"{""openrouter"":{""openai/gpt-5.4-mini"":{""name"":""GPT-5.4 Mini""}}}";
    ...
    public void ModelsDevSource_Parse_MapsCostPricing()
    {
        ...
        var json = @"{""openai"":{""gpt-4o"":{""cost"":{""input"":0.0000025,""output"":0.00001}}}}";
        var doc = System.Text.Json.JsonDocument.Parse(json);
        var result = ModelsDevMetadataSource.Parse(doc.RootElement);
        Assert.NotEmpty(result);
        var gpt4o = result.First();
    ...
    {
        var json = @"{""openai"":{""gpt-4o"":{""pricing"":{""input"":0.0000025,""output"":0.00001}}}}";
        var doc = System.Text.Json.JsonDocument.Parse(json);
        var result = ModelsDevMetadataSource.Parse(doc.RootElement);
        Assert.NotEmpty(result);
        var gpt4o = result.First();
...
internal sealed class StaticOverlaySource : IModelMetadataSource
{
    ...
    public StaticOverlaySource(IReadOnlyList<ModelMetadata> entries) => _entries = entries;
    public ValueTask<IReadOnlyList<ModelMetadata>> LoadAsync(CancellationToken ct = default) => new(_entries);
}
