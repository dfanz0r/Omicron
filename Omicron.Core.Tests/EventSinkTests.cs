...
public class EventSinkTests
{
    ...
    public void OmicronEvent_HasIdSequenceTimestampAndSession()
    {
        ...
        var evt = new SessionStartedEvent(new EventEnvelope(EventId.New(), 7, DateTimeOffset.UtcNow, sessionId),
            AgentId.New(),
            "gpt-4o",
            "openai");
    ...
    {
        var sessionId = SessionId.New();
        var evt = new UserMessageEvent(new EventEnvelope(EventId.New(), 1, DateTimeOffset.UtcNow, sessionId),
            "Hello"u8);
    ...
    {
        var sessionId = SessionId.New();
        var evt = new AssistantTextDeltaEvent(new EventEnvelope(EventId.New(), 1, DateTimeOffset.UtcNow, sessionId),
            "Hello"u8,
            null);
    ...
    public void AssistantResponseCompleteEvent_StoresFullResponse()
    {
        ...
        var evt = new AssistantResponseCompleteEvent(
            new EventEnvelope(EventId.New(), 1, DateTimeOffset.UtcNow, sessionId),
            "Full response"u8,
            "reasoning"u8,
    ...
    public void ToolInvocationStartedEvent_StoresArguments()
    {
        ...
        var evt = new ToolInvocationStartedEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, sessionId),
    ...
    public void InMemoryEventSink_Emit_StoresEvent()
    {
        ...
        var sessionId = SessionId.New();
        var evt = new SessionStartedEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, sessionId),
            AgentId.New(),
            "gpt-4o",
            "openai");
        ...
        IReadOnlyList<OmicronEvent> all = sink.GetAllEvents();
        Assert.Single(all);
    }
    ...
    public void InMemoryEventSink_GetSessionEvents_FiltersBySession()
    {
        ...
        sink.Emit(new SessionStartedEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, session1),
            AgentId.New(),
            "gpt-4o",
            "openai"));
        sink.Emit(new SessionStartedEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, session2),
            AgentId.New(),
            "claude-3",
            "anthropic"));

        IReadOnlyList<OmicronEvent> session1Events = sink.GetSessionEvents(session1);
        Assert.Single(session1Events);

        IReadOnlyList<OmicronEvent> session2Events = sink.GetSessionEvents(session2);
    ...
    public void InMemoryEventSink_Clear_RemovesAllEvents()
    {
        var sink = new InMemoryEventSink();
        var sessionId = SessionId.New();
        sink.Emit(new SessionStartedEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, sessionId),
            AgentId.New(),
            "gpt-4o",
            "openai"));
    ...
    public void InMemoryEventSink_EmitBatch_ReturnsStampedEvents()
    {
        ...
        var events = new List<OmicronEvent>
        {
            new SessionStartedEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, sessionId),
                ...
                "m1",
                "p1"),
            new UserMessageEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, sessionId),
                "Hello"u8),
            new TurnStartedEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, sessionId),
                "Hello")
        };

        IReadOnlyList<OmicronEvent> stamped = sink.EmitBatch(events);
    ...
    public void InMemoryEventSink_SequencesAreMonotonic()
    {
        ...
        sink.Emit(new SessionStartedEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, s2),
            agentId,
            "m2",
            "p2"));
        sink.Emit(new UserMessageEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, sessionId),
            "Hello"u8));
        sink.Emit(new AssistantTextDeltaEvent(new EventEnvelope(EventId.New(), 0, DateTimeOffset.UtcNow, s2),
            "Delta"u8,
            null));
        ...
        IReadOnlyList<OmicronEvent> sessionEvents = sink.GetSessionEvents(sessionId);
