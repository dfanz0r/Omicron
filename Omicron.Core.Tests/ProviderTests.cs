...
public class FakeProvider : IChatProvider
{
    ...
    public async IAsyncEnumerable<StreamEvent> StreamAsync(
        ...
        ChatOptions options)
    {
        var responseFunc = Responses.Count > 0 ? Responses[0] : null;
        ...
        {
            result = new LlmResult { Text = "Default fake response" };
        }
...
public class ProviderTests
{
    ...
    {
        var factory = new ProviderFactory();
        var openAi = factory.GetProvider("openai");
        var anthropic = factory.GetProvider("anthropic");
    ...
    {
        var factory = new ProviderFactory();
        var ex = Assert.Throws<KeyNotFoundException>(() => factory.GetProvider("nonexistent"));
    ...
    public void ProviderFactory_Register_CustomProvider()
    {
        ...
        Assert.True(factory.TryGetProvider("my-custom", out var retrieved));
    ...
    public async Task FakeProvider_StreamAsync_YieldsEvents()
    {
        ...
        var model = new Model { Id = "test", ProviderName = "fake" };
        var messages = new List<Message> { Message.UserMessage("Hi") };

        var events = new List<StreamEvent>();
        await foreach (var evt in provider.StreamAsync(model, messages, null, null, new ChatOptions()))
        {
            events.Add(evt);
        }

        Assert.Contains(events, e => e.Type == StreamEventType.TextDelta && e.Delta == "Hello from fake!");
        Assert.Contains(events, e => e.Type == StreamEventType.Done);
        var done = events.First(e => e.Type == StreamEventType.Done);
    ...
    public async Task FakeProvider_CompleteAsync_ReturnsCorrectResult()
    {
        ...
        ((FakeProvider)provider).Responses.Add(() => Task.FromResult(new LlmResult
        {
            Text = "Four!",
            ToolCalls = new List<ToolCallContent>
            {
                new("call_1", "calculator", new Dictionary<string, object?> { ["expression"] = "2+2" })
            },
        ...
        var model = new Model { Id = "test", ProviderName = "fake" };
        var messages = new List<Message> { Message.UserMessage("What's 2+2?") };

        var result = await provider.CompleteAsync(model, messages, null, null, new ChatOptions());
    ...
    public async Task FakeProvider_StreamAsync_WithToolCalls()
    {
        ...
        provider.Responses.Add(() => Task.FromResult(new LlmResult
        {
            ...
            ToolCalls = new List<ToolCallContent>
            {
                new("tc_1", "calculator", new Dictionary<string, object?> { ["expression"] = "2+2" })
            },
            StopReason = StopReason.ToolUse
        }));

        var model = new Model { Id = "test", ProviderName = "fake" };
        var messages = new List<Message> { Message.UserMessage("2+2?") };
        ...
        var toolCallEnd = events.FirstOrDefault(e => e.Type == StreamEventType.ToolCallEnd);
    ...
    public async Task FakeProvider_CompleteAsync_ErrorStopReason()
    {
        ...
        var model = new Model { Id = "test", ProviderName = "fake" };
        var result = await provider.CompleteAsync(model, new List<Message>(), null, null, new ChatOptions());
