...
public class ModelCatalogTests
{
    private sealed class SingleProviderRegistry : IProviderRegistry
    {
        ...
        public void Register(string name, IChatProvider provider) => _providers[name] = provider;
        public IChatProvider GetProvider(string name) => _providers[name];
        public bool TryGetProvider(string name, out IChatProvider? provider) => _providers.TryGetValue(name, out provider);
    }
    ...
    public async Task OpenRouterDiscovery_OnlyMarksZeroPricedModelsAsFree()
    {
        ...
        registry.Register("openrouter", new OpenRouterProvider(new HttpClient(new JsonHandler(json))));
    ...
    public async Task OpenRouterProvider_FetchModels_ParsesPricingIntoEntries()
    {
        ...
        Assert.Equal(1.5e-7, model.PromptCost, precision: 12);
        Assert.Equal(6.0e-7, model.CompletionCost, precision: 12);
    }
}
