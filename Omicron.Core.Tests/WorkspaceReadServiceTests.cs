...
public class WorkspaceReadServiceTests : IDisposable
{
    ...
    public async Task ReadFile_ReturnsContent()
    {
        ...
        WorkspaceReadContent content = await _service.ReadAsync("test.txt");
        WorkspaceFileContent file = Assert.IsType<WorkspaceFileContent>(content);
    ...
    public async Task ReadFile_WithChunk()
    {
        ...
        WorkspaceReadContent chunk0 = await _service.ReadAsync("big.txt", new ReadOptions
        {
            Chunk = 0,
            Limit = 3
        });
        WorkspaceFileContent f0 = Assert.IsType<WorkspaceFileContent>(chunk0);
        Assert.Equal(1, f0.Lines[0].Number);

        // Chunk 1 should start significantly later (not in the first 100 lines)
        WorkspaceReadContent chunk1 = await _service.ReadAsync("big.txt", new ReadOptions
        {
            Chunk = 1,
            Limit = 3
        });
        WorkspaceFileContent f1 = Assert.IsType<WorkspaceFileContent>(chunk1);
        Assert.NotEmpty(f1.Lines);
        Assert.True(f1.Lines[0].Number > 100,
            $"Expected chunk 1 to start well after line 100, got line {f1.Lines[0].Number}");

        // Offset relative to chunk: offset=1 should give the same as just chunk
        WorkspaceReadContent chunk1off1 = await _service.ReadAsync("big.txt",
            new ReadOptions
            {
                Chunk = 1,
                Offset = 1,
        ...
        WorkspaceFileContent f1o1 = Assert.IsType<WorkspaceFileContent>(chunk1off1);
        ...
        WorkspaceReadContent chunk1off2 = await _service.ReadAsync("big.txt",
            new ReadOptions
            {
                Chunk = 1,
                Offset = 2,
        ...
        WorkspaceFileContent f1o2 = Assert.IsType<WorkspaceFileContent>(chunk1off2);
        Assert.NotEmpty(f1o2.Lines);
        Assert.Equal(f1.Lines[0].Number + 1, f1o2.Lines[0].Number);

        // Chunk far past EOF returns empty
        WorkspaceReadContent chunkFar = await _service.ReadAsync("big.txt", new ReadOptions
        {
            Chunk = 99999
        });
        WorkspaceFileContent fFar = Assert.IsType<WorkspaceFileContent>(chunkFar);
    ...
    public async Task ReadDirectory_TruncatesAtMax()
    {
        ...
        WorkspaceReadContent content = await _service.ReadAsync("");
        WorkspaceDirectoryContent dir = Assert.IsType<WorkspaceDirectoryContent>(content);
    ...
    public async Task ReadBinary_ReturnsBinaryContent()
    {
        ...
        WorkspaceReadContent content = await _service.ReadAsync("image.png");
    ...
    {
        WorkspaceReadContent content = await _service.ReadAsync("nonexistent.txt");
        WorkspaceReadErrorContent err = Assert.IsType<WorkspaceReadErrorContent>(content);
    ...
    {
        WorkspaceReadContent content = await _service.ReadAsync("../../outside.txt");
        WorkspaceReadErrorContent err = Assert.IsType<WorkspaceReadErrorContent>(content);
    ...
    public async Task Renderer_FileOutput_IncludesHeaders()
    {
        ...
        WorkspaceReadContent content = await _service.ReadAsync("test.txt");
        WorkspaceReadResult result = new WorkspaceLlmTextRenderer().Render(content);
    ...
    public async Task Renderer_BinaryOutput()
    {
        ...
        WorkspaceReadContent content = await _service.ReadAsync("img.png");
        WorkspaceReadResult result = new WorkspaceLlmTextRenderer().Render(content);
    ...
    {
        WorkspaceReadContent content = await _service.ReadAsync("missing.txt");
        WorkspaceReadResult result = new WorkspaceLlmTextRenderer().Render(content);
