...
public class SessionResumeTests
{
    private static Model MakeModel(
        ...
        ApiType apiType = ApiType.OpenAiChat)
    {
    ...
    public async Task ResumeSession_HydratesMessages()
    {
        ...
        AgentSession original = host.CreateSession(model, "You are a test.");
        await foreach (OmicronEvent _ in original.PromptAsync("Hello")) { }

        // Resume from store
        AgentSession? resumed = await host.ResumeSessionAsync(original.Id, model);
        Assert.NotNull(resumed);
        Assert.Equal(original.Id, resumed.Id);
        Assert.NotEmpty(resumed.Messages);
        Assert.Contains(resumed.Messages,
            m => m.Role == MessageRole.User && m.GetTextString() == "Hello");
    ...
    public async Task ForkSession_CreatesNewSessionId()
    {
        ...
        AgentSession original = host.CreateSession(model);
        await foreach (OmicronEvent _ in original.PromptAsync("Hi")) { }

        AgentSession? forked = await host.ForkSessionAsync(original.Id, model);
    ...
    {
        var host = new OmicronHost(Environment.CurrentDirectory);
        AgentSession? result = await host.ResumeSessionAsync(SessionId.New(), MakeModel());
        Assert.Null(result);
    }
    ...
    public async Task ResumeSession_CanContinueConversation()
    {
        ...
        AgentSession original = host.CreateSession(model);
        await foreach (OmicronEvent _ in original.PromptAsync("First")) { }
        ...
        await foreach (OmicronEvent e in resumed.PromptAsync("Second"))
        {
            events.Add(e);
        }
    ...
    {
        var host = new OmicronHost(Environment.CurrentDirectory);
        Model model = MakeModel("gpt-5.5", "openai", ApiType.OpenAiResponses);
        ...
        Model otherModel = MakeModel("claude-3", "anthropic", ApiType.AnthropicMessages);
        AgentSession? forked = await host.ForkSessionAsync(original.Id, otherModel);
        Assert.NotNull(forked);
        Assert.NotEmpty(forked.Messages);
        Assert.Equal(original.Messages.Count, forked.Messages.Count);
    }
    ...
    public async Task ForkSession_TranscriptIsDurable()
    {
        ...
        AgentSession original = host.CreateSession(model);
        await foreach (OmicronEvent _ in original.PromptAsync("Fork durability test")) { }
        ...
        await foreach (
            OmicronEvent e in host.SessionStore.ReadEventsAsync(forked.Id, EventSequenceRange.All)
        )
        {
            forkedEvents.Add(e);
        }
        ...
        AgentSession? resumed = await host.ResumeSessionAsync(forked.Id, model);
        Assert.NotNull(resumed);
    ...
    public async Task ResumeSession_ProviderState_ReKeyedToNewAgentId()
    {
        ...
        AgentSession original = host.CreateSession(model);
        await foreach (OmicronEvent _ in original.PromptAsync("Test provider state")) { }

        // Resume and check that the provider state is accessible under the new AgentId
        AgentSession? resumed = await host.ResumeSessionAsync(original.Id, model);
        Assert.NotNull(resumed);

        var key = ProviderStateKey.Create(resumed.Id,
            resumed.AgentId,
            model.ProviderName,
            model.Id,
            model.ApiType);
        ProviderTurnState? state = host.ProviderStateManager.Get(key);
