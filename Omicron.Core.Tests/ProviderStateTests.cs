...
public class ProviderStateTests
{
    private static AgentSession CreateSession(
        Model model,
        IEventSink sink,
        IProviderStateManager? psm = null)
    {
        var ts = new ToolRegistry();
        var ps = new AllowAllPermissionService();
        IProviderStateManager p = psm ?? new ProviderStateManager(new InMemoryProviderConversationStateStore(), sink);
    ...
    public void ProviderStateKey_Create_NormalizesProviderNameCase()
    {
        ...
        var lower = ProviderStateKey.Create(sessionId,
            agentId,
            "openai",
            "gpt-4o",
            ApiType.OpenAiChat);
        var mixed = ProviderStateKey.Create(sessionId,
            agentId,
            "OpenAI",
            "gpt-4o",
            ApiType.OpenAiChat);
        var upper = ProviderStateKey.Create(sessionId,
            agentId,
            "OPENAI",
            "gpt-4o",
            ApiType.OpenAiChat);
    ...
    public void ProviderStateKey_ScopesBySessionAgentProviderModelApiType()
    {
        var sessionId = SessionId.New();
        var agentId = AgentId.New();
        var key1 = ProviderStateKey.Create(sessionId,
            agentId,
            "openai",
            "gpt-4o",
            ApiType.OpenAiChat);
        var key2 = ProviderStateKey.Create(sessionId,
            agentId,
            "openai",
            "gpt-4o",
            ApiType.OpenAiChat);
        var key3 = ProviderStateKey.Create(sessionId,
            agentId,
            "anthropic",
            "claude-3",
    ...
    public void InMemoryProviderConversationStateStore_SetAndGet()
    {
        ...
        store.Set(state);
        ProviderTurnState? retrieved = store.Get(key);
    ...
    public void InMemoryProviderConversationStateStore_IsCaseInsensitive()
    {
        ...
        ProviderTurnState? retrieved = store.Get(
            new ProviderStateKey(sessionId, agentId, "OPENAI", "gpt-4o", ApiType.OpenAiChat));

        Assert.NotNull(retrieved);
        Assert.Equal("resp_1", retrieved!.PreviousResponseId);

        store.Clear(new ProviderStateKey(sessionId, agentId, "openai", "gpt-4o", ApiType.OpenAiChat));

        Assert.Null(store.Get(new ProviderStateKey(sessionId, agentId, "OPENAI", "gpt-4o", ApiType.OpenAiChat)));
    }
    ...
    public void InMemoryProviderConversationStateStore_ClearSessionRemovesAllKeysForSession()
    {
        ...
        store.Set(new ProviderTurnState(
            ...
            null,
            null));
        store.Set(new ProviderTurnState(ProviderStateKey.Create(sessionId,
                agentId,
                "openai",
                "gpt-4o-mini",
                ApiType.OpenAiChat),
        ...
        Assert.Null(store.Get(ProviderStateKey.Create(sessionId,
            agentId,
            "openai",
            "gpt-4o-mini",
            ApiType.OpenAiChat)));
    ...
    public void ProviderStateManager_EmitsEventsOnSetAndClear()
    {
        ...
        var sessionId = SessionId.New();
        var key = ProviderStateKey.Create(sessionId,
            AgentId.New(),
            "openai",
            "gpt-4o",
            ApiType.OpenAiChat);
        ...
        IReadOnlyList<OmicronEvent> log2 = sink.GetAllEvents();
    ...
    public void ProviderStateManager_Set_NormalizesKey()
    {
        ...
        var rawKey = new ProviderStateKey(sessionId,
            AgentId.New(),
            "OpenAI",
            "gpt-4o",
            ApiType.OpenAiChat);
        ...
        ProviderTurnState result = manager.Set(state);
        ...
        ProviderTurnState? retrieved = manager.Get(ProviderStateKey.Create(sessionId,
            result.Key.AgentId,
            "OPENAI",
            "gpt-4o",
            ApiType.OpenAiChat));
        ...
        IReadOnlyList<OmicronEvent> log = sink.GetAllEvents();
        ProviderStateUpdatedEvent updatedEvent = log.OfType<ProviderStateUpdatedEvent>().Single();
    ...
    public void ProviderStateManager_Clear_DoesNotEmitForMissingKey()
    {
        ...
        IReadOnlyList<OmicronEvent> log = sink.GetAllEvents();
        ProviderStateClearedEvent clearEvent = log.OfType<ProviderStateClearedEvent>().Single();
    ...
    public void ProviderStateManager_SetAndClear_IncludeReason()
    {
        ...
        manager.Set(new ProviderTurnState(key, "resp_1", null, null, null), "continuation");
        ...
        sink.Clear();
        manager.Clear(key, "reset");

        IReadOnlyList<OmicronEvent> log2 = sink.GetAllEvents();
        ProviderStateClearedEvent clearEvent = log2.OfType<ProviderStateClearedEvent>().Single();
    ...
    public void InMemoryProviderConversationStateStore_ClearSession_ReturnsRemovedKeys()
    {
        ...
        IReadOnlyList<ProviderStateKey> removed = store.ClearSession(sessionId);
    ...
    public void ProviderStateManager_ClearSession_EmitsOneEventPerKeyWithReason()
    {
        ...
        manager.Set(new ProviderTurnState(
            ...
            null,
            null));
        manager.Set(new ProviderTurnState(ProviderStateKey.Create(sessionId,
                agentId,
                "openai",
                "gpt-4o-mini",
                ApiType.OpenAiChat),
        ...
        sink.Clear();
        manager.ClearSession(sessionId, "reset_all");
    ...
    public void ProviderStateManager_Set_PreservesStoragePolicy()
    {
        ...
        manager.Set(state, "test");

        ProviderTurnState? retrieved = manager.Get(key);
        Assert.NotNull(retrieved);
        Assert.Equal(ProviderStoragePolicy.AllowProviderStoredState, retrieved!.StoragePolicy);
        Assert.Equal("resp_123", retrieved.PreviousResponseId);
    }
    ...
    public async Task AgentSession_PersistsResponseIdOnDone()
    {
        ...
        var providerStateManager = new ProviderStateManager(new InMemoryProviderConversationStateStore(),
            eventSink);
        ...
        AgentSession session = CreateSession(model, eventSink, providerStateManager);
        ...
        var key = ProviderStateKey.Create(session.Id,
            ...
            "gpt-5.5",
            ApiType.OpenAiResponses);
        ProviderTurnState? state = providerStateManager.Get(key);
        Assert.NotNull(state);
        Assert.Equal("resp_abc123", state!.PreviousResponseId);

        // Verify event was emitted
        IReadOnlyList<OmicronEvent> log = eventSink.GetSessionEvents(session.Id);
        Assert.Contains(log, e => e is ProviderStateUpdatedEvent);
    }
    ...
    public async Task ResponsesModel_SupportsPreviousResponseIdFalse_DoesNotPersist()
    {
        ...
        var model = new Model
        {
            ...
            ApiType = ApiType.OpenAiResponses,
            StoragePolicy = ProviderStoragePolicy.AllowProviderStateNoStore,
            Compatibility = new ProviderCompatibility
            {
