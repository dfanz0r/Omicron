...
public sealed class TuiApiKeyPrompt : ITuiWidget
{
    ...
    public void Render(RenderContext context)
    {
        ...
        // Title
        string title = $" Enter API key for {ProviderName}: ";
        context.DrawText(dialogX + 1,
            dialogY + 1,
    ...
    {
        if (IsCompleted || string.IsNullOrEmpty(text))
        {
            return;
        }
    ...
    private void RemoveLastRune()
    {
        ...
        ReadOnlySpan<char> span = _input.ToString().AsSpan();
        if (
            Rune.DecodeLastFromUtf16(span, out _, out int charsConsumed) == OperationStatus.Done
            && charsConsumed > 0
        )
        {
